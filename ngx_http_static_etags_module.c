...
static char *
ngx_http_static_etags_hash_method(ngx_conf_t *cf, ngx_command_t *cmd,
                                  void *conf);
...
static char *ngx_http_static_etags_merge_loc_conf(ngx_conf_t *cf, void *parent,
                                                  void *child) {
...
//    struct stat                         stat_result;
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, log, 0, "[etag] working? \"%d\"",
                   loc_conf->enable);
    ...
    if (loc_conf->enable)
    {
            ...
            NGX_OK)
        {
            ...
            str_buffer.len =
                    ngx_sprintf(str_buffer.data, "%V_%T_%z", &r->uri, of.mtime,
                                of.size) - str_buffer.data;
            ...
            if (loc_conf->enable_hash)
            {
                ...
                } else if (ngx_strcmp(loc_conf->hash_method.data, "sha1") ==
                           0)
                {
